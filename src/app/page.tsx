...
const FAQS = [
  ...
  },
  {
    ...
    a: "Bitcoin and Ethereum are both holdings, sitting mid-table rather than at the top of it, but they are deliberately not scored on the equity moat framework — a monetary protocol's durability doesn't come from the same places a business's does. Crypto is scored on five protocol pillars: network effects, Schelling-point status, credible neutrality, regulatory incumbency, and security budget. Each protocol declares which pillar is its actual moat and that one carries 30% of the score — the heaviest single pillar, but not more than the other four combined — so Bitcoin scores on credible neutrality and Ethereum on network effects rather than being averaged through pillars that don't define them. Growth and valuation are scored exactly as they are for any other asset. Moat scores are not comparable across asset classes: Bitcoin's 94 measures protocol durability, which is not the same thing as an equity moat of 94.",
  },
  {
    q: "Why no gold?",
    a: "Gold is fully analyzed and scores a composite of 73 — respectable, but 69th in the universe and short of the 80-point inclusion threshold. Commodities are scored on three pillars — absolute scarcity, monetary history, industrial utility — with the declared primary pillar carrying 40%, so gold scores on its monetary history rather than being dragged down by thin industrial demand. Its moat comes out at 70. What keeps it out is growth of 68: gold produces no earnings, revenue, or cash flow and cannot compound. Bitcoin carries the same hard-money and inflation-hedge thesis with network effects and digital divisibility, and scores 82 — but the two now score identically on growth at 68, because Bitcoin's adoption CAGR was re-anchored in July 2026 to the rates its holder and address series actually show rather than an assumed long-run number. Bitcoin's remaining edge over gold is its moat, 94 against 70, and a slightly better entry on valuation. Gold stays in the coverage universe and earns a position the moment its composite clears the threshold and outranks the weakest holding.",
  },
  {
